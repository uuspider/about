...
在/etc下查找5分钟内被修改过属性的文件和目录。-amin代指访问时间，-mmin代指文件内容。

    $ find /etc -size +163840 -a -size -204800
    
在/etc下查找大于80MB小于100MB的文件。-a表示两个条件同时满足，-o表示满足其中任意一个条件即可。

    $ find /etc -name inittab -exec ls -l {} \;
    
在/etc下查找inittab文件并显示其详细信息，-exec表示对find命令的结果进行操作，类似的也可以用-ok；{} \;是固定用法，代指find命令的结果。
