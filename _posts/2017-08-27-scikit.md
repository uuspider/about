...
\\[ \sigama (z) = \frac{1}{1+e^{-z}} \\]

该函数值取值在0到1之间，z=0时，\\(\sigama = 0.5\\)，z减小时，\\(\sigama\\)快速逼近0，z增大时，\\(\sigama\\)快速逼近1。

把回归方程作为z带入Logistic函数中，可以得到一个0到1之间的数值，当该值大于0.5时，实例被归入1类，小于0.5时归入0类，这样就实现了分类的功能。
