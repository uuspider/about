...
\\[ Ent ( X ) = - \Sigma ^n _i=1 P ( x _i ) \log_2 P ( x _i ) \\]

熵越小，表示集合的纯度越高，如果集合所有元素属于同一类，则：

\\[ Ent ( X ) = - \Sigma ^n _i=1 1 \log_2 1 = 0 \\]
...
\\[ Gini_index(X,a) = \Sigma ^v _{ \upsilon =1} \frac{ \vert X^{\upsilon} \vert }{ \vert X \vert } Gini( X^{\upsilon} ) \\]
