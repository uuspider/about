...
输出为一个矩阵，称为混淆矩阵(Confusion Matrix)：
...
knn算法中提到的`score`, `classification_report`， `confusion_matrix`等在此也同样适用。
...
分类决策树的`score`, `classification_report`， `confusion_matrix`等在此也同样适用。
