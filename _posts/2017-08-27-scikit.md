...
默认为取其当前列有效数值的平均值：
    ...
    full_feature_value=Imputer(missing_values=-1, strategy='mean', axis=0)
    full_feature_value.fit_transform(X)
...
特征工程需要人的参与，更多地需要个人的经验，能否尽可能减少人的参与和监督，直接将数据交给机器进行处理，然后自动对不同特征赋予不同权重呢？神经网络可以做到这一点。关于神经网络，请查看[这里](http://about.uuspider.com/2017/10/10/keras.html){:target="_blank"}。
