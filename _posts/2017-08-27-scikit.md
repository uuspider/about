...
\\[ GainRatio ( X, a ) = \frac{ Gain( X, a )}{ IV(a) } \\]
...
\\[ GiniIndex (X,a) = \Sigma ^v _{ \upsilon =1} \frac{ \vert X^{\upsilon} \vert }{ \vert X \vert } Gini( X^{\upsilon} ) \\]

采用信息增益、增益率和基尼指数的决策树分别为ID3、C4.5和CART算法。下面即将讲述的`sklearn.tree.DecisionTreeClassifier`使用的是优化的CART，设置`criterion`参数为`entropy`(信息增益)，可近似为ID3算法，C4.5基于信息增益率，scikit-learn不能直接实现。
...
### 决策树 {#decision_tree_main}

决策树所采用的数据集格式与kNN相同，训练集中最后一列为标签，分离特征矩阵和标签向量：
...
`class_weight`， 指定样本各类别的权重，防止训练集某些类别的样本过多，导致决策树过于偏向这些类别。默认为'None'，可指定为'balanced'，设定算法自动调整权重，样本量少的类别对应的样本权重会提高。也可以分别指定各个样本的权重。
...
knn算法中提到的`classification_report`， `confusion_matrix`等在此也同样适用。

### 决策树的可视化 {#decision_tree_graph}

决策树经过可视化处理后，其结构会更加清晰，便于理解。决策树可视化一般采用graphviz
