...
在这条命令中，管道符左侧的命令为文本加上了行号，但是行号是自成一行，因此，我们使用`N`命令，首先将当前行的下一行存到辅缓存区，然后在主缓存区中把当前行的换行符`\n`替换为制表符`\t`，最后把辅缓存区中的内容追加到主缓存区之后。

如果需要删除所有换行，使用`sed 'N;s/\n/\t/g'`并不能实现预期的效果，简单的，可以使用`tr`：

    $ sed '=' /etc/passwd | tr -t '\n' '\t'

### 标签

如果使用`sed`实现删除所有换行的功能，就需要引入标签跳转功能进行流程控制：

    $ sed '=' /etc/passwd | sed ':label;N;s/\n/\t/;b label'

这是一个循环结构，`:label`是一个标签，`b label`指示命令**无条件**跳转到"label"标签，"标签"可以任意命名，且与`b`之间的空格也不是必需的。

除了`b`跳转，还有`t`跳转和`T`跳转(`b`和`t`分别是`branch`和`test`的缩写)，如：

    $ sed '{/false/tlabel;s/false/FALSE/;:label;s/nologin/NOLOGIN/}' /etc/passwd

如果'fasle'匹配成功，则按顺序执行，即先执行`s/false/FALSE/`，再执行`s/nologin/NOLOGIN/`；如果'fasle'匹配不成功，则跳转到指定标签，直接执行`s/nologin/NOLOGIN/`。

    $ sed '{/false/Tlabel;s/false/FALSE/;:label;s/nologin/NOLOGIN/}' /etc/passwd

如果'fasle'匹配成功，则跳转到指定标签，直接执行`s/nologin/NOLOGIN/`；如果'fasle'匹配不成功，则按顺序执行，即先执行`s/false/FALSE/`，再执行`s/nologin/NOLOGIN/`。

如果`b`、`t`、`T`之后不指定标签，则会跳过此后的所有命令，直接跳转到末尾：

    $ sed '{/false/t;s/false/FALSE/;s/home/HOME/}' /etc/passwd

即如果'fasle'匹配成功，则按顺序执行，即先执行`s/false/FALSE/`，再执行`s/home/HOME/`；如果'fasle'匹配不成功，后边的命令都不再执行。
