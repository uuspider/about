...
大写的AWK是一种编程语言，具有完整的变量、运算与判断、函数、流程控制、输入输出等等功能，而小写的`awk`是一个文本处理工具，和`grep`、`sed`并称为Linux文本处理的三剑客。

## 结构化文本 {#structure}

`awk`认为文件都是结构化的，由字符串和分隔符组成，这里的“分隔符”默认是空格和Tab，每个非空的字符串叫做“域”，`$1`，`$2`...分别表示左起第一个域、第二个域...，`$0`表示全部域。
