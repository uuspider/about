...
参考：[GNU grep][ref2]{:target="_blank"}, [GNU sed][ref3]{:target="_blank"}, [GNU awk][ref4]{:target="_blank"},   [Regular-Expressions.info][ref1]{:target="_blank"}, [One-line scripts for sed][ref5]{:target="_blank"}
...
大写的AWK是一种编程语言，具有完整的变量、运算与判断、函数、流程控制、输入输出等等功能，而小写的`awk`特指在Linux中与`grep`、`sed`并列的一个文本处理工具。

## 格式化文本 {#structure}

`awk`认为文件都是格式化的，每一行称为一个记录，一个记录由字符串和分隔符组成，这里的“分隔符”默认是空格和Tab，每个非空的字符串叫做“域”，`$1`，`$2`...分别表示左起第一个域、第二个域...，`$0`表示全部域，也就是当前记录、当前行。
...
这条命令中用到了数组，还用到了`for`循环，`awk`的功能强大，正是因为它具有完整的变量和流程控制。
