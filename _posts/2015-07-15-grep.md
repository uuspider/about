...
参考：[Regular-Expressions.info][ref1]{:target="_blank"}, [GNU grep][ref2]{:target="_blank"}, [GNU sed][ref3]{:target="_blank"}, [One-line scripts for sed][ref4]{:target="_blank"}
...
[ref4]:https://linuxtoy.org/archives/sed-awk.html
...
*   [sed 基础](#sed)
    *   [sed 替换](#s)
    *   [sed 选项](#sedoption)
    *   [sed 动作](#cmd): [添加](#ai)\|[替换](#c)\|[删除](#d)\|[打印](#p)
    *   [位置参数](#address)
...
<br>
也可以先把动作编写成文件，然后调用该文件执行动作，此时需使用选项`-f`：
...
`grep`和`sed`都是按行处理文本的，如果我们要读取文件的某一列，应该怎么做呢？

    $ cut -f1,5-7 -d':' /etc/passwd
    
这条命令将打印出第1,5,6,7列。`cut`的选项包括：
...
大写的AWK是一种编程语言，具有完整的变量、运算与判断、函数、流程控制、输入输出等等功能，而小写的`awk`特在Linux中与`grep`、`sed`并列的一个文本处理工具。

## 格式化文本 {#structure}

`awk`认为文件都是格式化的，每一行称为一个记录，一个记录由字符串和分隔符组成，这里的“分隔符”默认是空格和Tab，每个非空的字符串叫做“域”，`$1`，`$2`...分别表示左起第一个域、第二个域...，`$0`表示全部域，也就是当前记录。
...
`$1 !~ /ro*t/ || NR==1`是执行条件，`$1 !~ /ro*t/ `是正则匹配，与后边的`NR==1`进行“或”运算。
...
这条命令中用到了数组，还用到了`for`循环，`awk`的功能强大就是因为它具有完整的变量和流程控制。
