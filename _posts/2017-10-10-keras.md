...
\\[ \begin{split} \vert \vert \sigma ^T _k \vert \vert & \leqslant \vert \vert \sigma ^T _t \vert \vert \prod ^{t-1} _{i=k} \vert \vert W \vert \vert \vert \vert diag [f' (net _i)] \vert \vert & \leqslant \vert \vert \sigma ^T _t \vert \vert (\beta _W \beta _f) ^{t-k} \end{split} \\]

式中\\(\beta\\)定义为矩阵的模的上界。当指数\\((t-k)\\)很大时候(即向前看很远的时候)，会引起误差项的值增长或缩小非常快，从而导致梯度爆炸或梯度消失。长短时记忆网络(LTSM,)就是为了解决这个问题而构建的。
